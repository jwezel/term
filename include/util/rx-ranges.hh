...
/*!
    ...
    void advance() noexcept; // Advance to the next position.

    output_type next() noexcept; // Get current value and advance to next position.

    bool at_end() const noexcept; // True if there are no more elements.

    size_t size_hint() const noexcept; // A hint about how many times `advance()` can be called.
    ...
    Calling `get()` or `advance()` while `at_end() == true` is a breach of contract, and is allowed to
...
struct is_input_range<
    ...
    std::void_t<
        ...
        decltype(std::declval<T&>().advance())>> : std::true_type {};
template <class T>
...
///
/// This calls R::advance_by() if available. Otherwise, it calls R::advance() at most n times.
template <class R>
...
struct iterator_range {
    ...
    constexpr output_type advance() noexcept {
        RX_ASSERT(!at_end());
        ++current_;
        return *current_;
    }
    ...
    [[nodiscard]] constexpr output_type next() noexcept {
        RX_ASSERT(!at_end());
        output_type result = *current_;
        advance();
        return result;
    }
...
struct vector_range {
    ...
    constexpr const output_type next() noexcept {
        RX_ASSERT(!at_end());
...
struct idempotent_range {
    ...
    constexpr output_type next() noexcept {
        RX_ASSERT(!at_end());
        auto result = *current_;
        advance();
        return result;
...
struct seq {
    ...
    [[nodiscard]] constexpr output_type next() {
        auto result = current;
        advance();
        return result;
    }
...
struct transform {
    ...
    struct Range {
        ...
        constexpr output_type next() {
            RX_ASSERT(!at_end());
            auto result = transform_.func(input_.get());
            advance();
            return result;
        }
...
struct ZipRange {
    ...
    constexpr void advance() noexcept {
        RX_ASSERT(!at_end());
        _advance(std::index_sequence_for<Inputs...>{});
    }
...
private:
    ...
    template <size_t... Index>
    constexpr void _advance(std::index_sequence<Index...>) noexcept {
        (std::get<Index>(inputs).advance(), ...);
    }
...
struct ChainRange {
    ...
    constexpr void advance() noexcept {
        return _advance(std::make_index_sequence<sizeof...(Rs)>());
    }
...
private:
    ...
    constexpr void _advance(std::index_sequence<>) noexcept {
        RX_ASSERT(false); // chain was at end!
    }

    template <size_t i, size_t... rest>
    constexpr void _advance(std::index_sequence<i, rest...>) noexcept {
        if (i == current_input) {
            ...
            _advance(std::index_sequence<rest...>{});
        }
    }
...
private:
    ...
    template <size_t... Index>
    constexpr void _advance(std::index_sequence<Index...>) noexcept {
        ((std::get<Index>(inputs).at_end() ? 0 : (std::get<Index>(inputs).advance(), 0)), ...);
    }
