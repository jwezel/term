    ...
    for (auto z1 = z - 1; z1 >= 0; --z1) {
      if (zorder[z1]->area.intersects(window->area)) {
        ...
        for (auto z2 = z1 + 1; z2 < int(zorder.size()); ++z2) {
          ...
          // Add screen updates for this split
          updates |= push_back(
            zwindow->fragments |
...
Updates Screen::reshapeWindow(Window *window, const Rectangle &area) {
  ...
  if (window->area != area) {
    ...
    const auto intersections{window->area - area};
    const auto damageAreas = views::concat(views::single(area), intersections);
      ...
      if (zorder[j]->area.intersects(searchArea)) {
        ...
        for (const auto &fragment: fragments) {
          for (const auto &damageArea: damageAreas) {
            ...
            if (intersection) {
              updates.emplace_back(Fragment{intersection.value(), window});
            }
          }
        }
      }
...
Updates Screen::text(Window *window, const Vector &position, const Text &text) {
  ...
  auto textArea{Rectangle(0, 0, text.width(), text.height()) + window->area.position()};
  return screenUpdates(
    window->fragments |
    views::transform([&textArea](const Rectangle &r) {return r & textArea;}) |
...
Vector Screen::relative(Window *window, const Vector &position) {
